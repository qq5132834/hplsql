...
import sics.seiois.res.plsql.functions.Function;
...
public class Package {
  ...
  HashMap<String, PlsqlParser.Create_function_stmtContext> func = new HashMap<String, PlsqlParser.Create_function_stmtContext>();
  HashMap<String, PlsqlParser.Create_procedure_stmtContext> proc = new HashMap<String, PlsqlParser.Create_procedure_stmtContext>();
   ...
   */
  void createSpecification(PlsqlParser.Create_package_stmtContext ctx) {
    int cnt = ctx.package_spec().package_spec_item().size();
    for (int i = 0; i < cnt; i++) {
      PlsqlParser.Package_spec_itemContext c = ctx.package_spec().package_spec_item(i);
   ...
   */
  void createBody(PlsqlParser.Create_package_body_stmtContext ctx) {
    int cnt = ctx.package_body().package_body_item().size();
    for (int i = 0; i < cnt; i++) {
      PlsqlParser.Package_body_itemContext c = ctx.package_body().package_body_item(i);
      if (c.declare_stmt_item() != null) {
        visit(c);
      }
   ...
   * Execute function
   */
  public boolean execFunc(String name, PlsqlParser.Expr_func_paramsContext ctx) {
    PlsqlParser.Create_function_stmtContext f = func.get(name.toUpperCase());
    if (f == null) {
   ...
   * Execute procedure
   */
  public boolean execProc(String name, PlsqlParser.Expr_func_paramsContext ctx, boolean traceNotExists) {
    PlsqlParser.Create_procedure_stmtContext p = proc.get(name.toUpperCase());
    if (p == null) {
