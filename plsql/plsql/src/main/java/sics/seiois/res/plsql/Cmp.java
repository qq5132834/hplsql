...
public class Cmp implements Runnable {
  ...
  Query query;
  String conn;
  PlsqlParser.Cmp_stmtContext ctx;
  ...
  Cmp(Exec e, PlsqlParser.Cmp_stmtContext c, Query q, String cn) {
    exec = e;  
    trace = exec.getTrace();
    info = exec.getInfo();
    ctx = c;
   ...
   * Run CMP command
   */
  Integer run(PlsqlParser.Cmp_stmtContext ctx) {
    trace(ctx, "CMP");
    this.ctx = ctx;
    timer.start();
   ...
   */
  String getSql(PlsqlParser.Cmp_stmtContext ctx, StringBuilder conn, int idx) throws Exception {
   ...
   */
  String getSelectList(PlsqlParser.Cmp_stmtContext ctx, String conn, String table, String query) throws Exception {
    StringBuilder sql = new StringBuilder();
