...
public class Select {
   ...
   */
  public Integer select(PlsqlParser.Select_stmtContext ctx) {
    if (ctx.parent instanceof PlsqlParser.StmtContext) {
    ...
    if (!(ctx.parent instanceof PlsqlParser.StmtContext)) {           // No need to execute at this stage
      exec.stackPush(sql);
      return 0;  
    }    
    if (trace && ctx.parent instanceof PlsqlParser.StmtContext) {
    ...
    try {
      ...
      else if (ctx.parent instanceof PlsqlParser.StmtContext) {
   ...
   */
  public Integer cte(PlsqlParser.Cte_select_stmtContext ctx) {
    int cnt = ctx.cte_select_stmt_item().size();
    StringBuilder sql = new StringBuilder();
    sql.append("WITH ");
    for (int i = 0; i < cnt; i++) {
      PlsqlParser.Cte_select_stmt_itemContext c = ctx.cte_select_stmt_item(i);
   ...
   * Part of SELECT
   */
  public Integer fullselect(PlsqlParser.Fullselect_stmtContext ctx) {
  ...
  public Integer subselect(PlsqlParser.Subselect_stmtContext ctx) {
    StringBuilder sql = new StringBuilder();
   ...
   * SELECT list 
   */
  public Integer selectList(PlsqlParser.Select_listContext ctx) {
    StringBuilder sql = new StringBuilder();
   ...
   * FROM clause
   */
  public Integer from(PlsqlParser.From_clauseContext ctx) {
    StringBuilder sql = new StringBuilder();
   ...
   */
  public Integer fromTable(PlsqlParser.From_table_name_clauseContext ctx) {
    StringBuilder sql = new StringBuilder();
   ...
   * Subselect in FROM
   */
  public Integer fromSubselect(PlsqlParser.From_subselect_clauseContext ctx) {
    StringBuilder sql = new StringBuilder();
    sql.append("(");
   ...
   * JOIN clause in FROM
   */
  public Integer fromJoin(PlsqlParser.From_join_clauseContext ctx) {
    StringBuilder sql = new StringBuilder();
    if (ctx.T_COMMA() != null) {
      sql.append(", ");
      sql.append(evalPop(ctx.from_table_clause()));
   ...
   */
  public Integer fromTableValues(PlsqlParser.From_table_values_clauseContext ctx) {
    StringBuilder sql = new StringBuilder();
   ...
   * WHERE clause
   */
  public Integer where(PlsqlParser.Where_clauseContext ctx) {
    boolean oldBuildSql = exec.buildSql; 
    exec.buildSql = true;
    StringBuilder sql = new StringBuilder();
   ...
   * Get INTO clause
   */
  PlsqlParser.Into_clauseContext getIntoClause(PlsqlParser.Select_stmtContext ctx) {
   ...
   */
  int getIntoCount(PlsqlParser.Select_stmtContext ctx) {
    PlsqlParser.Into_clauseContext into = getIntoClause(ctx);
    if (into != null) {
      return into.ident().size();
    }
    List<PlsqlParser.Select_list_itemContext> sl = ctx.fullselect_stmt().fullselect_stmt_item(0).subselect_stmt().select_list().select_list_item();
   ...
   */
  String getIntoVariable(PlsqlParser.Select_stmtContext ctx, int idx) {
    PlsqlParser.Into_clauseContext into = getIntoClause(ctx);
    if (into != null) {
      return into.ident(idx).getText();
    }
    PlsqlParser.Select_list_itemContext sl = ctx.fullselect_stmt().fullselect_stmt_item(0).subselect_stmt().select_list().select_list_item(idx);
    if (sl != null) {
   ...
   */
  public Integer option(PlsqlParser.Select_options_itemContext ctx) {
    if (ctx.T_LIMIT() != null) {
