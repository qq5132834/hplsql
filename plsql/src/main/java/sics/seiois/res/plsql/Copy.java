...
public class Copy {
   ...
   * Run COPY command
   */
  Integer run(PlsqlParser.Copy_stmtContext ctx) {
    trace(ctx, "COPY");
    initOptions(ctx);
   ...
   * @throws Exception 
   */
  void copyToTable(PlsqlParser.Copy_stmtContext ctx, Query query) throws Exception {
   ...
   * @throws Exception 
   */
  void copyToFile(PlsqlParser.Copy_stmtContext ctx, Query query) throws Exception {
    ResultSet rs = query.getResultSet();
    if (rs == null) {
      return;
    }
   ...
   */
  public Integer runFromLocal(PlsqlParser.Copy_from_local_stmtContext ctx) {
   ...
   */
  void initOptions(PlsqlParser.Copy_stmtContext ctx) {
    int cnt = ctx.copy_option().size();
    for (int i = 0; i < cnt; i++) {
      PlsqlParser.Copy_optionContext option = ctx.copy_option(i);
   ...
   */
  void initFileOptions(List<PlsqlParser.Copy_file_optionContext> options) {
    srcSizeInBytes = 0;
    for (PlsqlParser.Copy_file_optionContext i : options) {
